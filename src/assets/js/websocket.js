...
export default function (opts) {
    ...
    function createWebSocket() {
        ...
        let host = null, url= null
        if (window['cordova']) {
            url = `wss://ws-exchange.gcox.com/ws${port}`
        } else {
            host = location.host.toLowerCase()
            url = `${Config.protocol}${Config.domain}/ws${port}`
        ...
        ws.onerror = function (e) {
            console.log(`error websocket:${port}`, 'evnet:' + e)
        }
        return ws
    }
